...
import io.github.dug22.cipherlabs.core.ciphers.algorithm.classic.symmetric.VigenereCipher;
...
public class VigenereCipherForm extends CipherForm {
    ...
    protected void initOptions() {
        ...
        vigenereVisualPane.setPrefHeight(0);
        ...
        centeredBottomBorderPane.addBottomContent(keyCombinationsTextFlow);
        centeredBottomBorderPane.addBottomContent(crackTimePerMillionTextFlow);
        ;
    ...
    protected void initListeners() {
        ...
        getEncryptButton().setOnAction((_) -> {
            ...
            initEncryptDecryptAction(vigenereCipher, keyTextField.getText(), true, "Vigenère Encrypted Result");
        ...
        getDecryptButton().setOnAction((_) -> {
            ...
            initEncryptDecryptAction(vigenereCipher, keyTextField.getText(), false, "Vigenère Decrypted Result");
            keyCombinationsResultText.setText("26^" + currentKey.length());
            crackTimePerMillionResultText.setText(String.format("%.2f seconds", Math.pow(26, currentKey.length()) / 1_000_000));
            executeVisual();
        });
    ...
    private void executeVisual() {
        vigenereVisualPane.setPrefHeight(250);
        ...
        visual.clear(this);
        visual.play();
        if (!centeredBottomBorderPane.getBottomContainer().getChildren().contains(vigenereVisualPane)) {
            centeredBottomBorderPane.getBottomContainer().getChildren().add(vigenereVisualPane);
        }
        Platform.runLater(() -> {

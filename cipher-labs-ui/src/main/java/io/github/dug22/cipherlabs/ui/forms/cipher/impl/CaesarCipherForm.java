...
import io.github.dug22.cipherlabs.core.ciphers.algorithm.classic.symmetric.CaesarCipher;
...
public class CaesarCipherForm extends CipherForm {
    ...
    protected void initOptions() {
        ...
        caesarVisualPane.setPrefHeight(0);
        ...
        centeredBottomBorderPane.addBottomContent(topAlphabetRow);
        centeredBottomBorderPane.addBottomContent(bottomAlphabetRow);
    ...
    private void initEncryptDecryptAction(boolean encrypt) {
        ...
        initEncryptDecryptAction(caesarCipher, keyTextField.getText(), encrypt, encrypt ? "Caesar Cipher Encrypted Result" : "Caesar Cipher Decrypted Result");

        CaesarCipherVisual visual = new CaesarCipherVisual(caesarVisualPane, this, caesarCipher.getSteps(), fromAlphabetTextField, toAlphabetTextField);
        VisualAnimationManager.terminate();
        if (!centeredBottomBorderPane.getBottomContainer().getChildren().contains(caesarVisualPane)) {
            centeredBottomBorderPane.getBottomContainer().getChildren().add(caesarVisualPane);
        }
        caesarVisualPane.setPrefHeight(75);
        visual.clear(this);
        visual.play(encrypt);
        Platform.runLater(() -> {
